...
Relations are never loaded automatically. They load on explicit request, and all FK lookups are batched with DataLoader. Loading 200 posts with their authors fires one `SELECT ... WHERE id IN (...)`, not 200 individual queries.
...
## Known limitations for the v1.0.0

- `ALTER COLUMN` for type changes is not generated by `SchemaDiff`. Column type migrations must be written by hand.
- The in-memory adapter supports a subset of SQL. Complex subqueries and window functions are not parsed.
- There is no lazy loading. All relation loading is explicit and opt-in.
